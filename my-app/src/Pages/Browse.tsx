...
const BrowsePage = ({ type }: { type: ResourceType }) => {
          ...
          <div className="w-full px-4 md:px-0 p-12">
              ...
              {items.map((item) => (
                      'original_title' in item ? (
                          <div key={item.id}>
                              <Moviebox movie_id={item.id} title={item.title} posterpath={item.poster_path}></Moviebox>
                          </div>
                      ) : (

                          <div key={item.id}>
                              <Showbox show_id={item.id} title={item.name} posterpath={item.poster_path}></Showbox>
                          </div>
                      )
                  ))}
              </div>
