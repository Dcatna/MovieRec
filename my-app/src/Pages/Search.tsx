  ...
  const { data, fetchNextPage, hasNextPage, isLoading, refetch } =
    useInfiniteQuery({
      ...
      queryFn: async ({ pageParam = 1 }) => {
        if (!search) {
          return { total_pages: 0, page: 0, data: [] }; // Handle empty search query
        }
        if (searchState === "movie") {
          if(search == "") {
            const data  = await client.fetchMovieList(
              pageParam,
              searchState,
              []
          ...
          if(search == ""){
            const data = await client.fetchShowList(
              pageParam,
              searchState,
              []
            )
      ...
      enabled: !!search, // Enable query only if search exists
    });


    useInfiniteScroller(0.95, !isLoading , async () => {
      if (hasNextPage) {
        try {
          await fetchNextPage();
        } catch (error) {
        ...
        <div className="grid lg:grid-cols-4 md:grid-cols-3 sm:grid-cols-2 gap-4 p-6">
          {items.map((item) => (
            <ContentListItem
              ...
              onClick={() =>
                navigate(`/${item.isMovie ? "movie" : "show"}/${item.id}`)
              }
            />
          ))}
        </div>
